...
import com.codelouders.shmetterling.example.auth.oauth2.{Token, OauthUser}
import com.codelouders.shmetterling.example.auth.oauth2.provider.AuthorizationProvider
  ...
  override def receive: Receive = {
    ...
    case Create(user) =>
      ...
      authProvider.login(user) match {
        case Some(usr) =>
          val token = Token(CryptoUtil.generateToken, System.currentTimeMillis() + SessionManager.LifeTimeInMilli)
